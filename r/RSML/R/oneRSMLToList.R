...
#' Import a single RSML file into a List
#' @param rsml.path    The path to the .rsml file. Mandatory.
#' @param store.raw     Do you wan to store the raw RSML data into the rsml object? Take much more space. Defaults to FALSE
#' @keywords rsml
#' @import XML 
#' @examples
#' oneRSMLToList()
oneRsmlToList <- function(rsml.path, store.raw = F){
  ...
  if(store.raw) rsml.data$raw[[name]] <- rsml
